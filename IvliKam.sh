...
#
# https://github.com/ivarmedi/piwebcam-ivli

# ensure we are running on a supported Raspbian release
if ! grep -q '10 (buster)' /etc/os-release; then
...
# URL of the project
export MY_URL="https://api.ivli.se"
# URL with the latest version available
MY_UPDATES_URL="https://api.ivli.se/firmware/version.txt"
...
IR_LED_PIN=26
# User agent string
USER_AGENT="$MY_NAME v$MY_VERSION"

# Device identifiers
export MY_HARDWARE_TYPE="$(awk '/^Hardware/{print $(NF)}' /proc/cpuinfo)"
export MY_HARDWARE_REVISION="$(awk '/^Revision/{print $(NF)}' /proc/cpuinfo)"
export MY_HARDWARE_ID="$(awk '/^Serial/{print $(NF)}' /proc/cpuinfo)"
export MY_MAC_ADDRESS="$(cat /sys/class/net/$IFACE/address)"
...
base64 -d <<<""
...
function save_config {
    # set default values
    if [[ -z "$DEVICE_INITIAL_SETUP_COMPLETED" ]]; then
        DEVICE_INITIAL_SETUP_COMPLETED="0"
    fi
    ...
    if [[ -z "$IVLI_ENABLE" ]]; then
        IVLI_ENABLE=0
    fi
    if [[ -z "$IVLI_KEEP_NOT_FOUND" ]]; then
        IVLI_KEEP_NOT_FOUND=1
    fi
...
# Wizard completed
DEVICE_INITIAL_SETUP_COMPLETED='$DEVICE_INITIAL_SETUP_COMPLETED'
...
# If checked, upon a motion the image will be further analyzed with an artifical intelligence model to detect cars and their license plates.
IVLI_ENABLE='$IVLI_ENABLE'
# The API key for authenticating against the IVLI service.
IVLI_TOKEN='$IVLI_TOKEN'
# The probability threshold to trigger an analysis (e.g. 0.8).
IVLI_THRESHOLD='$IVLI_THRESHOLD'
# Keep pictures and videos not containing vehicles on the camera.
IVLI_KEEP_NOT_FOUND='$IVLI_KEEP_NOT_FOUND'
...
function installer {
    ...
    if ! grep  -q "$MY_FILE" $STARTUP_FILE; then
        ...
        apt-get update
        apt-get -y install busybox dnsmasq hostapd f2fs-tools lighttpd motion php-fpm php-cgi php-gd zip sharutils ssmtp jq mpack autossh wiringpi php-markdown initramfs-tools-core bootlogd
...
function test_ivli_connection {
    IVLI_TOKEN=${1:-$IVLI_TOKEN}

    IVLI_OUTPUT=$(curl -s -X POST \
      -A "${USER_AGENT}" \
      -H "Authorization: Key $IVLI_TOKEN" \
      -H "Content-Type: application/json" \
      -o- -d@- \
      https://api.ivli.se/v1/test << FILEIN
      {
          "device": {
              "id": "${MY_HARDWARE_ID}"
          },
...
FILEIN
    ...
    STATUS_CODE=`echo $IVLI_OUTPUT| jq '.statusCode'`
...
function register_ivli_device {
    IVLI_OUTPUT=$(curl -s -X POST \
      -A "${USER_AGENT}" \
      -H "Authorization: Key $IVLI_TOKEN" \
      -H "Content-Type: application/json" \
      -o- -d@- \
      https://api.ivli.se/v1/register_device << FILEIN
      {
          "device": {
              "id": "${MY_HARDWARE_ID}",
              "name": "${DEVICE_NAME}",
              "mac": "${MY_MAC_ADDRESS}",
              "hardware": "${MY_HARDWARE_TYPE}",
              "revision": "${MY_HARDWARE_REVISION}",
              "minimum_confidence": "${IVLI_THRESHOLD}"
          },
          "software": {
              "version": "${MY_VERSION}"
          }
...
if [ "$1" = "test_ivli_connection" ]; then
    apikey=$2
    if test_ivli_connection $apikey; then
        echo -n "OK"
    else
        echo -n "ERROR"
    fi
...
if [ "$1" = "register_ivli_device" ]; then
    register_ivli_device
fi
...
function configure_network {
    ...
    if [[ "$WIFI_MODE" == "CLIENT" ]]; then
        log "Setting client wifi mode with SSID $WIFI_CLIENT_SSID"
...
function configure_camera {
    ...
    if [[ $(eval $INTERNET) = "1" ]] && [[ $IVLI_ENABLE == 1 ]]; then
        register_ivli_device
    fi
...
if [ "$1" = "motion" ]; then
    ...
    if [[ -n "$FILENAME" && $FILENAME != *"/.snapshots/"*  ]]; then
        ...
        if [[ $(eval $INTERNET) == "1" ]]; then
            ...
            if [[ $IVLI_ENABLE == 1 && -n "$IVLI_TOKEN" && -n "$IVLI_THRESHOLD" ]]; then
                log "Uploading $FILENAME to IVLI for analysis"
                IVLI_OUTPUT=$(curl -s -X POST \
                -A "${USER_AGENT}" \
                -H "Authorization: Key $IVLI_TOKEN" \
                -H "Content-Type: application/json" \
                -o - \
                -d @- https://api.ivli.se/v1/upload << FILEIN
                {
                  "image": {
...
FILEIN
                ...
                STATUS_CODE=`echo $IVLI_OUTPUT| jq '.statusCode'`
                if [[ $STATUS_CODE == 200 ]]; then
                    log "IVLI upload succeeded"
                else
                    log "IVLI service failed"
                fi
            fi
            # notify the user
